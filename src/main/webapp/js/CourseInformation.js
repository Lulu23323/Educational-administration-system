...
$(function(){
    ...
    $(document).on("click",".addButton_ok",function(){
        ...
        var str='<tr class="trHover">' +
            ...
            '<td class="couId">'+arrAdd[0]+'</td>' +
            '<td class="couName">'+arrAdd[1]+'</td>' +
            '<td class="couAcademy">'+arrAdd[2]+'</td>' +
            '<td class="couMajor">'+arrAdd[3]+'</td>' +
            '<td class="couIntroduction">'+arrAdd[4]+'</td>' +
            '<td>' + '<a class="viewInf ">View Details</a> <a class="updateInf ">Modify</a></td>' +
            '<td><button id="open_btn" class="btn">Grade</button></td>'
            '</tr>'
        $("#tdata").append(str)
        //奇偶行的颜色不同
    ...
    $(document).on("click",".updatabtn_ok",function(){
        ...
        $.ajax({
                 type:"POST",
                 url:"/2009/updateCourseServlet", //提交修改学生的网址
                 data:{
                 	"ChangeCourseID":arrMove[0],
                 	"ChangeCourseName":arrMove[1],
                 	"ChangeCourseAcademy":arrMove[2],
                 	"ChangeCourseMajor":arrMove[3],
                 	"ChangeCourseInfor":arrMove[4],
                 	
                 },//提交的数据
                 success:function(res){
                     console.log("传送修改课程数据成功");
                     console.log(res);
                 },
                 error:function(){
                     alert("向后台传输修改课程数据出错");
    ...
    $(function(){
        ...
        $("#deleteButton").click(function() {
            ...
            $("input:checked").each(function() { // 遍历选中的checkbox
            	...
            	delCourseID.push(delP.find(".couId").text());            	
            ...
            $.ajax({
	                type:"POST",
	                url:"/2009/deleteCourseServlet", //提交删除课程的网址
	                data:{
	                	"delCourseID":delCourseID.join(",")
	                },//提交的数据
	                success:function(res){
...
//grade
    ...
    function show1(e) {
        ...
        var courseID = e.parentElement.parentElement.children[1].innerText;
	    $.ajax({
	            type:"POST",
	            url:"/2009/showSingleCourseGradeServlet", //展示课程下学生成绩
	            dataType:"json",
	            data:{
	            	"courseID":courseID
	            	},
