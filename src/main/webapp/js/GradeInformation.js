...
$(function(){
    ...
    $(document).on("click",".addButton_ok",function(){
        ...
        var str='<tr class="trHover">' +
            ...
            '<td class="stuCourse">'+arrAdd[4]+'</td>' +
            '<td class="stuGrade">'+arrAdd[5]+'</td>' +
    ...
    $(document).on("click",".updatabtn_ok",function(){
        ...
        $.ajax({
                 type:"POST",
                 url:"/2009/updateGradeServlet", //提交修改成绩的网址
                 data:{
                  "ChangeStuID":arrMove[0],
                  "ChangeStuName":arrMove[1],
                  "ChangeStuAca":arrMove[2],
                  "ChangeStuMaj":arrMove[3],
                  "ChangeStuCouID":arrMove[4],
                  "ChangeStuGra":arrMove[5]
                  
                 },//提交的数据
                 success:function(res){
                     console.log("传送修成绩数据成功");
                     console.log(res);
                 },
                 error:function(){
                     alert("向后台传输修改成绩数据出错");
        ...
        $("#deleteButton").click(function() {
		   ...
		   var delCouID = [];
            $("input:checked").each(function() { // 遍历选中的checkbox
             ...
             delCourseID.push(delP.find(".stuCourse").text());
            ...
            $.ajax({
                 type:"POST",
                 url:"/2009/deleteGradeServlet", //提交删除成绩的网址
                 data:{
                  "delStuID":delStuID.join(","),
                  "delStuCourse":delCouID.join(",")
                 },//提交的数据
                 success:function(res){
                     console.log("传送删学生id成绩成功");
                     console.log(res);
                 },
                 error:function(){
                     alert("向后台传输删除学生id成绩出错");
                 }
             })   
